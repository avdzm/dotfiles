...
local function factory(args)
    ...
    local tempfile = args.tempfile or "/sys/class/hwmon/hwmon2/temp1_input"  --"/sys/devices/virtual/thermal/thermal_zone0/temp"
    local settings = args.settings or function() end

    function temp.update()
        helpers.async({"find", "/sys/class/", "-type", "f", "-name", "*temp*"}, function(f)
            temp_now = {}
            ...
            coretemp_now = temp_now[tempfile/1000] or "N/A"
            widget = temp.widget
            settings()
        end)
    end

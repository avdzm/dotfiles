...
mytextclock.font = "Ubuntu Mono Bold 16"
...
-- Volume
local volume = awful.widget.watch('bash -c ' .. os.getenv("HOME") .. '"/.local/bin/volume.sh"',1)
...
-- Net speed 
local netspeed = awful.widget.watch('bash -c ' .. os.getenv("HOME") .. '"/.local/bin/netspeed.sh wlp3s0"',1)
	...
	function (widget, stdout)
      ...
      if string.match(w, "^.-:") == "TotalTime:" then 
		...
		elseif mocp_now.State=="PAUSE" then
			...
			widget:set_text("Not playing") -- Not playing  
		end
...
awful.screen.connect_for_each_screen(function(s)
    ...
    beautiful.taglist_font = "Font Awesome 5 Free Solid Bold 13"
...
globalkeys = gears.table.join(
    ...
    awful.key({ "Mod1",           }, "Shift_L",
        function ()
          awful.spawn(os.getenv("HOME") .. "/.local/bin/keyboardLayout.sh")
        end,
    ...
    awful.key({ modkey, 		}, "i",			function () awful.spawn(os.getenv("HOME") .. "/.rofi_scripts/rofi_online_radio.sh") end,
